...
class Evaluator(object):
    ...
    def evaluate_video(self, model, prop, prompt, vi_data, output_path):
        imgs = vi_data['imgs']
        ...
        # inference
        t0 = time.time()
        pred_imgs = model(imgs, neg_masks, pos_masks, self.res, prop, prompt)
        t1 = time.time()
    ...
    def evaluate(self, model, prop, prompt, output_path):
        model.cuda()
        vi_data = self.dataset.get_video()
        os.makedirs(os.path.join(output_path, self.seq), exist_ok=True)
        seconds, frames = self.evaluate_video(model, prop, prompt, vi_data, output_path)
